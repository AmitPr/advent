...
class opc:
    ...
    def op(self):
        ...
        p_per_op = {1:3,2:3,3:1,4:1,5:2,6:2,7:3,8:3,9:1,99:0}
        ...
        #Add(1)/Multiply (2)
        if op==1 or op ==2:
            self.inst[self.inst[self.ptr+3]+pOffset[2]]= p1 + p2 if op ==1 else p1 * p2
            self.ptr+=4
        #Input(3)
        elif op==3:
            x = 0
...
if __name__=="__main__":
    panel = {}
    computer = opc(sys.argv[1], returnOut = True)
    computer.inst[0]=2
    _,output = computer.run()
    for i in range(0,len(output),3):
        x = output[i]
        y = output[i+1]
        t = output[i+2]
        panel[(x,y)]=t
    print(list(panel.values()).count(2))
    computer = opc(sys.argv[1], returnOut = True,inputOnly=True)
    computer.inst[0]=2
    panel = np.zeros((21,38),dtype=int).tolist()
    segment =0
    while True:
        bp = p = 0
        for j in panel:
            ...
            if 4 in j:
                p = j.index(4)
        code,output=computer.run([np.sign(p-bp)],True)
        for i in range(0,len(output),3):
            x = output[i]
            y = output[i+1]
            t = output[i+2]
        ...
        if code == -1:
            break
        print(segment,sum([x.count(2) for x in panel]), end = '    \r')
    print('\n' + str(segment))
