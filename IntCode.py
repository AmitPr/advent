...
class opc:
    ...
    def op(self):
        ...
        #Add(1)/Multiply (2)
        if op==1 or op ==2:
            self.inst[self.param(self.ptr+3,params[2]]= p1 + p2 if op ==1 else p1 * p2
            self.ptr+=4
        #Input(3)
        elif op==3:
            x = 0
            if self.curArg <len(self.inputs):
                ...
                x = int(input('in: '))
            self.inst[self.param(self.ptr+1,params[0])]=x
            self.ptr+=2
        #Print(4)
        elif op==4:
            if self.returnOut:
        ...
        #Less than(7)/Equal to(8)
            ...
            if (p1 < p2 and op == 7) or (p1==p2 and op == 8):
                self.inst[self.param(self.ptr+3,params[2]]=1
            else:
                self.inst[self.param(self.ptr+3,params[2]]=0
            self.ptr+=4
        #Relative bases
        elif op==9:
